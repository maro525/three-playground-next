...
const TriangleView = () => {
  ...
  const update = () => {
    ...
    c.current.raycaster.setFromCamera(c.current.mouse, camera)
    const intersects = c.current.raycaster.intersectObject(c.current.plane)
  ...
  const onMouseMove = (event) => {
    c.current.mouse.x = (event.clientX / window.innerWidth) * 2 - 1
    c.current.mouse.y = -(event.clientX / window.innerWidth) * 2 + 1
  }
